...
class CompleteTimelinePipeline:
    ...
    def write_final_report(self, report_data: List[Dict], sheet_name: str = "Final Report", 
                          end_date: str = "2025-10-01", end_odometer: float = 172000):
        ...
        # Filter 2: Remove trips to/from Markham
        print(f"\n🔍 Filtering out Markham trips...")
        non_markham_trips = []
        removed_markham = []
        ...
        for trip in report_data:
            ...
            # Check if either address contains "markham"
            if 'markham' in start_addr or 'markham' in dest_addr:
                removed_markham.append(trip)
                print(f"   ⚠️  Removed Markham trip: {trip.get('Date')} {trip.get('Start Time', '').split()[1] if ' ' in trip.get('Start Time', '') else ''}")
                print(f"       {trip.get('Starting Point', '')[:50]}... → {trip.get('Destination', '')[:50]}...")
            else:
                non_markham_trips.append(trip)
        
        if removed_markham:
            print(f"   ❌ Removed {len(removed_markham)} trips to/from Markham")
        else:
            print(f"   ✅ No Markham trips found")
        
        report_data = non_markham_trips
        
        # Filter 3: Remove trips to/from Bolton
        print(f"\n🔍 Filtering out Bolton trips...")
        non_bolton_trips = []
        removed_bolton = []
        ...
        for trip in report_data:
            ...
            # Check if either address contains "bolton"
            if 'bolton' in start_addr or 'bolton' in dest_addr:
                removed_bolton.append(trip)
                print(f"   ⚠️  Removed Bolton trip: {trip.get('Date')} {trip.get('Start Time', '').split()[1] if ' ' in trip.get('Start Time', '') else ''}")
                print(f"       {trip.get('Starting Point', '')[:50]}... → {trip.get('Destination', '')[:50]}...")
            else:
                non_bolton_trips.append(trip)
        
        if removed_bolton:
            print(f"   ❌ Removed {len(removed_bolton)} trips to/from Bolton")
        else:
            print(f"   ✅ No Bolton trips found")
        
        report_data = non_bolton_trips
        
        # Filter 4: Remove trips from May 11-15, 2025
        print(f"\n🔍 Filtering out May 11-15, 2025 trips...")
        non_may_trips = []
        removed_may = []
        
        from datetime import datetime
        may_start = datetime.strptime('2025-05-11', '%Y-%m-%d')
        may_end = datetime.strptime('2025-05-15', '%Y-%m-%d')
        ...
        for trip in report_data:
            ...
            if trip_date_str:
                try:
                    ...
                    if may_start <= trip_date <= may_end:
                        removed_may.append(trip)
                        print(f"   ⚠️  Removed May 11-15 trip: {trip.get('Date')} {trip.get('Start Time', '').split()[1] if ' ' in trip.get('Start Time', '') else ''}")
                        print(f"       {trip.get('Starting Point', '')[:50]}... → {trip.get('Destination', '')[:50]}...")
                    else:
                        non_may_trips.append(trip)
                except:
                    non_may_trips.append(trip)
            else:
                non_may_trips.append(trip)
        ...
        if removed_may:
            print(f"   ❌ Removed {len(removed_may)} trips from May 11-15, 2025")
        else:
            print(f"   ✅ No trips found in May 11-15, 2025 date range")
        
        report_data = non_may_trips
        ...
        # Cities for "Travel to Customer Site"
        customer_site_cities = [
            'arthur', 'port elgin', 'kincardine', 'harriston', 'tiverton',
            'minto', 'paisley', 'caledon village', 'georgetown', 'saugeen shores'
        ]
        # Cities for "Meeting with Customers"
        meeting_cities = [
            'mississauga', 'toronto', 'barrie',
            'woodbridge', 'cambridge', 'listowel', 'etobicoke'
        ]
        # Cities for "Delivery"
        delivery_cities = ['niagara']
...
def main():
    ...
    GOOGLE_MAPS_API_KEY = "AIzaSyBEIsPKDYB685wyt1TZfCS2HFEAfKDF62o"
    SHEETS_CREDENTIALS_PATH = "credentials.json"
    SPREADSHEET_ID = "1TouI_wd8H1Ko2eroIdKO1ZrYmhgihU2H45tgQaGEmPk"
    SERVICE_ACCOUNT_EMAIL = "timeline-converter@gen-lang-client-0653846390.iam.gserviceaccount.com"
